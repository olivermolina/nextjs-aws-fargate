...
export const JWT_EXPIRES_IN = 3600 * 24 * 7; // 7 days
...
export const decode = (token: string): any => {
  ...
  const formatSignature = signature.replace(/ /g, '+');

  if (verifiedSignature !== signature && verifiedSignature !== formatSignature) {
...
export const verify = (token: string, privateKey: string): Record<string, any> => {
  ...
  if (verifiedSignature !== signature && verifiedSignature !== signature.replace(' ', '+')) {
    throw new Error('Invalid signature');
  }
